...
test("brainstorm replaces the supplied prompt with an executable writing contract", () => {
  assert.ok(!brainstorm.includes(originalPrompt));
  assert.match(brainstorm, /Account for each applicable surface/iu);
  assert.match(brainstorm, /Completion\s+requires every observation/iu);
});
