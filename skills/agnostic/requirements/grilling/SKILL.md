...
Work the tree in **rounds**. The **frontier** is every decision whose prerequisites are already settled — the questions you can ask _now_ without guessing at answers you haven't heard yet. Ask the whole frontier in one round: number each question and give your recommended answer. Each numbered question must state one decision, the explicit question, a concise recommendation, and why it is preferred; when interpretations differ, require a precise choice. Then wait for the user's answers before the next round.
