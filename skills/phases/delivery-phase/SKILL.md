---
name: delivery-phase
description: Routes a scoped delivery goal from planning through implementation, review, debugging, docs ingest, and closeout.
...
2. Inspect only enough goal, plan, notes, diff, review, validation, and docs state to choose the current gate.
...
- **Resume:** user returns after manual planning, implementation, review, debugging, or docs work.
...
- A phase may delegate to `create-plan`, `implement-spec`, `review-phase`, `debugging-phase`, `docs-ingest-phase`, or `stack` only from its own phase file.
