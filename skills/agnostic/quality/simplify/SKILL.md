...
2. **Combine related concepts**: If two types, functions, or constants overlap significantly, merge them. The fewer distinct concepts a reader must hold in their head, the better. _Example: two union types sharing 3 of 4 values → one type with shorter value names._

3. **Derivability**: If a value can be computed from other values already in scope, don't pass or store it separately. Removing derivable state often simplifies signatures, types, and control flow in one move. _Example: an `isDirty` parameter that's always derived from computing `editorContent !== baseline` can be dropped._
