...
5. If image generation is unavailable, fall back to prototype artifacts and record the fallback.
6. For prototype intent, produce prototype artifacts; do not fall back to images.
7. Use `repo-asset-management` for durable visual links before approval or backlog handoff when assets are generated.

## Rules

- Child-skill delegation is phase-local: `design-taste-frontend`, `gpt-taste`, `image-to-code`, `imagegen-frontend-web`, and `imagegen-frontend-mobile` belong here only when needed.
